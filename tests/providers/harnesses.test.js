...
test('fake Claude, Codex, and Ollama harnesses make CI deterministic', () => {
  ...
  assert.equal(preflight('codex:gpt-5.6:medium', { host: 'codex', permissionMode: 'default' }, run).ok, true);
...
test('real provider smoke tests are opt-in', { skip: process.env.BDFL_SMOKE !== '1' }, () => {
  const codex = preflight(process.env.BDFL_SMOKE_MODEL || 'codex:gpt-5.6:medium', { host: 'codex', permissionMode: 'read-only' });
