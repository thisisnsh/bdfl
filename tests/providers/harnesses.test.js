...
test('fake Claude, Codex, and Ollama harnesses make CI deterministic', () => {
  assert.equal(preflight('claude:sonnet', { host: 'claude', permissionMode: 'default' }, run).ok, true);
  assert.equal(preflight('codex:gpt-5.6-sol', { host: 'codex', permissionMode: 'default' }, run).ok, true);
  assert.equal(preflight('ollama:qwen3.5:9b', {
...
test('real provider smoke tests are opt-in', { skip: process.env.BDFL_SMOKE !== '1' }, () => {
  const codex = preflight(process.env.BDFL_SMOKE_MODEL || 'codex:gpt-5.6-sol', { host: 'codex', permissionMode: 'read-only' });
