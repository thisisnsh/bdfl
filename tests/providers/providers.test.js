...
test('builds exact Codex JSONL invocation with medium effort', () => {
  const invocation = buildInvocation('codex:gpt-5.6-sol', {
    host: 'codex', permissionMode: 'default', prompt: 'do work'
  });
  assert.equal(invocation.command, 'codex');
  assert.deepEqual(invocation.args, [
    'exec', '--json', '-m', 'gpt-5.6-sol', '-c', 'model_reasoning_effort="medium"', '--sandbox', 'workspace-write', 'do work'
  ]);
});

test('uses the parent host Ollama harness without losing tag colons', () => {
  const invocation = buildInvocation('ollama:qwen3.5:9b', {
...
test('resumes exact provider sessions instead of writing answers to stdin', () => {
  const codex = buildResumeInvocation('codex:gpt-5.6-sol', { host: 'codex', permissionMode: 'default', sessionId: 'thread-1', prompt: 'v2' });
  assert.deepEqual(codex.args.slice(0, 3), ['exec', 'resume', '--json']);
  assert.deepEqual(codex.args.slice(-2), ['thread-1', 'v2']);
  const claude = buildResumeInvocation('claude:sonnet', { host: 'claude', permissionMode: 'default', sessionId: 'session-1', prompt: 'Approve' });
...
test('preflight exposes missing executables and models as visible states', () => {
  const missing = preflight('codex:gpt-5.6-sol', { host: 'codex', permissionMode: 'default' }, () => ({ status: 1 }));
  ...
  const model = preflight('ollama:qwen3.5:9b', {
...
test('preflight exposes authentication failures without fallback', () => {
  ...
  const result = preflight('codex:gpt-5.6-sol', { host: 'codex', permissionMode: 'default' }, fakeRun);
