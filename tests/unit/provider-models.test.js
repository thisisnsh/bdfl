...
test('returns the exact ordered built-in catalogs', () => {
  ...
  assert.deepEqual(
    codexCatalog(),
    ['gpt-5.6-sol', 'gpt-5.6-terra', 'gpt-5.6-luna', 'gpt-5.5', 'gpt-5.4', 'gpt-5.4-mini'].map(entry)
  );
});
...
test('discovers provider executables and queries Ollama for installed models', () => {
  ...
  const io = {
    accessSync(file) {
      probes.push(file);
      if (!file.endsWith('codex') && !file.endsWith('ollama'))
        throw Object.assign(new Error('missing'), { code: 'ENOENT' });
    },
    readFileSync() {
      throw new Error('account files must not be read');
    }
  };
  const calls = [];
  const run = (...args) => {
