...
test('routes plan rename and deletion to the owning repository and fans out bulk deletion', (t) => {
  ...
  const renamed = lineages.rename(secondPlan.lineage.planId, 'Mango release'); assert.equal(renamed.name, 'Mango release'); assert.equal(renamed.repositoryRoot, fs.realpathSync(mango)); assert.equal(lineages.load(secondPlan.lineage.planId).name, 'Mango release');
