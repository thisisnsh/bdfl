...
test('classifies every child state and aggregates parents without active styling', () => {
  const state = fixture();
  const [lead, worker, verifier] = state.sessions;
  ...
  assert.equal(
    parentVisualState([lead, worker], { runningSessionIds: new Set(['lead']), activeSessionId: 'lead' }),
    'working'
  );
...
test('working and focus styles remain steady without animation', () => {
  ...
  assert.match(visualStyle('active').ansi, new RegExp(STYLE.bgYellow.replace('[', '\\[')));
  assert.match(visualStyle('active').ansi, new RegExp(STYLE.black.replace('[', '\\[')));
  assert.equal(visualStyle('active').inverse, true);
});

test('renders one-row top chrome, a right-aligned internal tip, and one-row bottom badges', () => {
  const layout = layoutChrome(fixture(), { columns: 100, rows: 12, title: 'Alpha session', activeAction: 'Plans' });
  const plain = layout.plainLines.join('\n');
  assert.match(
    plain,
    /bdfl - Alpha session \[Star\] \[Report issues\].*\[New\] \[Plans\] \[Sessions\] \[Reviews\] \[Close\]/
  );
  assert.doesNotMatch(plain, /Quit/);
  assert.match(plain, /Ctrl\+C twice quits/);
  assert.equal(layout.tipRow, layout.frame.bottom - 1);
  assert.equal(layout.parentRow, layout.frame.bottom);
  assert.equal(layout.childRow, layout.frame.bottom);
  assert.equal(layout.links.find((item) => item.link === 'report').url, ISSUE_URL);
  assert.equal(layout.links.find((item) => item.link === 'repository').url, REPOSITORY_URL);
  assert.match(layout.lines[0], new RegExp(STYLE.bgYellow.replace('[', '\\[')));
  assert.doesNotMatch(layout.lines[0], new RegExp(STYLE.underline.replace('[', '\\[')));
...
test('uses exclusive active highlighting across creation-stable open agent tabs', () => {
  const state = fixture();
  const layout = layoutChrome(state, {
    columns: 100,
    rows: 10,
    expandedWorkstreamId: 'one',
    activeSessionId: 'worker',
    activeAction: 'Reviews',
    runningSessionIds: new Set(['lead'])
  });
  assert.equal(layout.activeSessionId, null);
  assert.equal(layout.actions.find((item) => item.action === 'Reviews').state, 'active');
  assert.equal(
    layout.children.some((item) => item.state === 'active'),
    false
  );
  ...
  const childLayout = layoutChrome(state, { columns: 100, rows: 10, activeSessionId: 'worker' });
  assert.equal(childLayout.children.find((item) => item.sessionId === 'worker').state, 'active');
  assert.equal(
    childLayout.actions.some((item) => item.state === 'active'),
    false
  );
});
...
test('bottom rail shows open agents across every session and excludes closed history', () => {
  ...
  state.sessions.find((item) => item.id === 'worker').status = 'paused';
  state.sessions.find((item) => item.id === 'worker').explicitlyClosed = true;
  let layout = layoutChrome(state, { columns: 100, rows: 10, activeSessionId: 'lead' });
  assert.deepEqual(layout.parents, []);
  assert.deepEqual(
    layout.children.map((item) => item.sessionId),
    ['other', 'other-worker', 'lead']
  );
  layout = layoutChrome(state, { columns: 100, rows: 10, activeSessionId: 'worker' });
  assert.deepEqual(
    layout.children.map((item) => item.sessionId),
    ['other', 'other-worker', 'lead', 'worker']
  );
});
...
test('all visible badges have exact hitboxes and narrow layouts stay inside the frame', () => {
  for (const columns of [100, 42, 20, 12]) {
    ...
    assert.equal(
      layout.plainLines.every((line) => textWidth(line) === columns),
      true
    );
    for (const hit of layout.hits) {
      const line = layout.plainLines[hit.row - 1];
      const value = [...line].slice(hit.start - 1, hit.end).join('');
      assert.equal(textWidth(value), hit.end - hit.start + 1);
      assert.ok(hit.start >= 2 && hit.end < columns);
    }
    assert.equal(
      stripAnsi(layout.output)
        .split('\n')
...
test('CJK and combining-mark names occupy terminal cells without shifting later badges or hits', () => {
  ...
  for (const columns of [70, 34, 20, 12]) {
    ...
    for (const hit of layout.hits) {
      const line = layout.plainLines[hit.row - 1];
      const rendered = [...layout.actions, ...layout.links, ...layout.parents, ...layout.children].find(
        (item) => item.row === hit.row && item.start === hit.start && item.end === hit.end
      );
