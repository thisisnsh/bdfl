...
test('parses file, hunk, and source coordinates and colors every wrapped patch row', () => {
  ...
  assert.ok(rows.filter((row) => row.sourceLine === 10).every((row) => row.file === 'new.js' && row.hunk === parsed[7].hunk));
...
test('mouse drags append ordered scoped ranges, highlight them, and preserve outside selection', () => {
  const view = new ReviewView({ columns: 24, viewportHeight: 40 }).open({ executionId: 'e', id: 'c', status: 'review', diff: PATCH });
  ...
  assert.deepEqual(view.selections(), [{ file: 'new.js', hunk: '@@ -1,2 +1,3 @@ function example()', startLine: 9, endLine: 10, text: '-const oldValue = a very long removed value;\n+const newValue = a very long replacement value;' }]);

  frame = view.render({ bodyTop: 5, bodyLeft: 3, bodyWidth: 24 });
  assert.ok(frame.rows.filter((row) => [9, 10].includes(row.sourceLine)).every((row) => row.selected && row.styled.includes(COLORS.inverse)));
  ...
  view.handleMouse({ final: 'm', button: 0, row: context.screenRow, column: 4 });
  assert.deepEqual(view.selections().map(({ startLine, endLine }) => [startLine, endLine]), [[9, 10], [11, 11]]);
  assert.equal(view.removeLastSelection(), true);
  assert.deepEqual(view.selections().map(({ startLine }) => startLine), [9]);
  assert.deepEqual(view.feedback('Fix this').selections.map(({ startLine }) => startLine), [9]);
...
test('ignores patch rows that cannot produce a normalized file-and-hunk selection', () => {
  const view = new ReviewView({ columns: 80, viewportHeight: 40 }).open({ executionId: 'e', id: 'c', status: 'review', diff: PATCH });
  assert.deepEqual(view.rangesForVisualSpan(0, 6), []);
  const hunk = renderPatch(PATCH, 80).find((row) => row.sourceLine === 8);
  assert.deepEqual(view.rangesForVisualSpan(hunk.visualLine, hunk.visualLine), [{ file: 'new.js', hunk: '@@ -1,2 +1,3 @@ function example()', startLine: 8, endLine: 8, text: '@@ -1,2 +1,3 @@ function example()' }]);
});

test('serializes all selections in selection order and renders durable feedback excerpts', () => {
  const item = { executionId: 'e', id: 'c', status: 'running', diff: PATCH, feedback: [{ at: 'now', message: 'Please revise', selections: [{ file: 'new.js', hunk: '@@ hunk', startLine: 9, endLine: 10, text: '-old\n+new' }] }] };
  const view = new ReviewView({ columns: 80, viewportHeight: 50 }).open(item);
  view.beginSelection(8); view.finishSelection(8);
  view.beginSelection(10); view.finishSelection(10);
  const payload = view.feedback('Fix both');
  assert.equal(payload.message, 'Fix both');
  assert.deepEqual(payload.selections.map(({ startLine }) => startLine), [9, 11]);
...
test('one scroll operation clamps line, wheel, page, resize, shrink, selection, and state transitions', () => {
  const long = { executionId: 'e', id: 'c', status: 'review', diff: `${PATCH}\n${Array.from({ length: 20 }, (_, index) => ` line ${index}`).join('\n')}` };
  ...
  view.beginSelection(0); view.finishSelection(0);
  assert.equal(view.state().scroll, 0);
...
test('keeps item state by durable identity and exposes immediate labels with distinct confirmations', () => {
  ...
  const labels = ['accepted', 'running', 'checking', 'verifying', 'failed', 'retrying', 'integration-queued', 'integrating', 'integration-conflict', 'integration-review', 'complete'].map((status) => stateDescriptor({ status, feedback: status === 'running' ? [{}] : [] }).label);
  assert.deepEqual(labels, ['Accepted', 'Feedback sent · Revising', 'Checking', 'Verifying', 'Failed', 'Retrying', 'Integrating', 'Integrating', 'Integration repair', 'Ready to integrate', 'Complete']);
  ...
  assert.deepEqual(stateDescriptor({ status: 'running', attention: true, feedback: [{}] }), { label: 'Needs response', tone: 'yellow', actions: ['feedback'] });
});
