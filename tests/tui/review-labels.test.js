...
function fixture(overrides = {}) {
  const state = {
    ...
    workstreams: [{ id: 'one', status: 'active', createdAt: '2026-01-01' }],
    sessions: [
      {
        id: 'd',
        workstreamId: 'one',
  ...
  const execution = {
    ...
    chunks: [
      {
        id: 'build-bash-lookup',
        status: 'review',
        summary: 'Added strict validation for every argument.',
        diff: 'diff --git a/planet.sh b/planet.sh\n--- a/planet.sh\n+++ b/planet.sh\n@@ -1 +1 @@\n-previous lookup\n+replacement lookup',
        attempts: [{ sessionId: 'w' }]
      }
    ]
  };
  ...
  const scheduler = {
    list: () => [execution],
    resume() {},
    feedback(_executionId, _chunkId, payload) {
  ...
  const supervisor = new TerminalSupervisor('/tmp/bdfl-review-labels', {
    ...
    lineage: {
      list: () => [
        { planId: 'plan-private-id', title: 'Planet lookup scripts', workstreamId: 'one', originSessionId: 'd' }
      ]
    },
    git: {},
    scheduler,
    ...
    output: {
      columns: overrides.columns || 48,
      rows: overrides.rows || 28,
      write(value) {
        writes.push(value);
      }
    },
  ...
  supervisor.acquire = () => {};
  supervisor.release = () => {};
  return { supervisor, handlers, writes, feedback, execution, state };
}

test('Review uses human plan and agent labels while hiding internal IDs', () => {
  const { supervisor, handlers } = fixture();
  supervisor.start();
  supervisor.activate('Reviews');
  handlers.get('data')('\r');
  const plain = supervisor
    .actionPageLines()
    .join('\n')
...
test('Review retains durable accepted workers and shows their squashed diff', () => {
    ...
    chunks: [
      {
        ...
        status: 'accepted',
        commit: 'head',
        diff: '+latest commit only',
        attempts: [{ sessionId: 'open', base: 'base' }]
      },
      {
        id: 'closed-result',
        status: 'accepted',
        commit: 'closed-head',
        attempts: [{ sessionId: 'closed', base: 'base' }]
      },
      {
        id: 'waiting-result',
        status: 'waiting',
        summary: 'Which output format should I use?',
        attempts: [{ sessionId: 'waiting', base: 'base' }]
      },
  ...
  const supervisor = new TerminalSupervisor('/tmp/bdfl-accepted-review-test', {
    ...
    git: {
      resultDiff(chunk) {
        diffs.push(chunk.id);
        return 'diff --git a/result b/result\n-old combined result\n+new combined result';
      }
    }
  });
  ...
  assert.deepEqual(
    ...
    ['open-result', 'closed-result', 'waiting-result', 'question-result']
  );
...
test('Review can accept or amend verifier remedies and keeps override separate', () => {
  ...
  const execution = {
    ...
    verification: { state: 'fail', summary: 'Tests failed' },
  ...
  assert.match(supervisor.actionPageLines().join('\n'), /f suggest[\s\S]*repair/);
  assert.match(supervisor.actionPageLines().join('\n'), /o override/);
  handlers.get('data')('r');
  assert.match(
    supervisor.actionPageLines().join('\n'),
  ...
  supervisor.activate('Reviews');
  handlers.get('data')('\r');
  handlers.get('data')('fPreserve the active terminal\r');
  assert.deepEqual(remedies, [['execution']]);
  handlers.get('data')('\r');
...
test('Review colors file metadata, hunks, removals, and additions through narrow wrapping', () => {
  const { supervisor, handlers } = fixture({ columns: 30 });
  supervisor.start();
  supervisor.activate('Reviews');
  handlers.get('data')('\r');
...
test('Review forwards body mouse selection and sends selected excerpts with feedback', () => {
  const { supervisor, handlers, feedback } = fixture({ columns: 60, rows: 34 });
  supervisor.start();
  supervisor.activate('Reviews');
  handlers.get('data')('\r');
  supervisor.draw();
  let frame = supervisor.reviewView.lastFrame;
  const removal = frame.rows.find((row) => row.patch && row.sourceLine === 5);
  const addition = frame.rows.find((row) => row.patch && row.sourceLine === 6);
  handlers.get('data')(
    `\u001b[<0;4;${removal.screenRow}M\u001b[<32;4;${addition.screenRow}M\u001b[<0;4;${addition.screenRow}m`
  );
  frame = supervisor.reviewView.lastFrame;
  const hunk = frame.rows.find((row) => row.patch && row.sourceLine === 4);
  handlers.get('data')(`\u001b[<0;4;${hunk.screenRow}M\u001b[<0;4;${hunk.screenRow}m`);
  handlers.get('data')('fFix both\r');
  assert.equal(feedback[0].message, 'Fix both');
  assert.deepEqual(feedback[0].selections, supervisor.reviewView.selections());
  assert.deepEqual(
    feedback[0].selections.map(({ startLine, endLine }) => [startLine, endLine]),
    [
      [5, 6],
      [4, 4]
    ]
...
test('Review exposes undo and clear controls for additive mouse selections', () => {
  const { supervisor, handlers, feedback } = fixture({ columns: 60, rows: 34 });
  supervisor.start();
  supervisor.activate('Reviews');
  handlers.get('data')('\r');
  ...
  let plain = supervisor
    ...
    .join('\n')
    .replace(/\u001b\[[0-9;?]*[A-Za-z]/g, '');
  assert.match(plain, /2 selected/);
  assert.match(plain, /u undo/);
  assert.match(plain, /x clear/);
  handlers.get('data')('u');
  assert.equal(supervisor.reviewView.selections().length, 1);
  handlers.get('data')('x');
  assert.deepEqual(supervisor.reviewView.selections(), []);
  handlers.get('data')('fFix replacement\r');
  assert.deepEqual(feedback[0].selections, []);
  supervisor.stop();

  const cleared = fixture({ columns: 60, rows: 34 });
  cleared.supervisor.start();
  cleared.supervisor.activate('Reviews');
  cleared.handlers.get('data')('\r');
  cleared.supervisor.draw();
  frame = cleared.supervisor.reviewView.lastFrame;
  const selected = frame.rows.find((row) => row.patch && row.sourceLine === 5);
  cleared.handlers.get('data')(`\u001b[<0;4;${selected.screenRow}M\u001b[<0;4;${selected.screenRow}m`);
  cleared.handlers.get('data')('x');
  assert.deepEqual(cleared.supervisor.reviewView.selections(), []);
  cleared.handlers.get('data')('fNo excerpt\r');
  assert.deepEqual(cleared.feedback[0].selections, []);
  cleared.supervisor.stop();
});

test('Review scrolling is bounded after wheel, page, width, and status changes', () => {
  const { supervisor, handlers, execution } = fixture({ columns: 32, rows: 16 });
  execution.chunks[0].diff += `\n${Array.from({ length: 30 }, (_, index) => ` context ${index}`).join('\n')}`;
  supervisor.start();
  supervisor.activate('Reviews');
  handlers.get('data')('\r');
  handlers.get('data')('\u001b[6~\u001b[6~');
  assert.ok(supervisor.reviewView.state().scroll <= supervisor.reviewView.maximumScroll());
  handlers.get('data')('\u001b[<65;10;8M');
  assert.ok(supervisor.reviewView.state().scroll <= supervisor.reviewView.maximumScroll());
  execution.chunks[0].status = 'accepted';
  execution.chunks[0].diff = '+short';
  supervisor.draw();
  assert.equal(supervisor.reviewView.state().scroll, supervisor.reviewView.maximumScroll());
  supervisor.stop();
});
