...
const LABELS = { claude: 'Claude Code', codex: 'Codex', ollama: 'Ollama', default: 'Claude current default', medium: 'Medium', low: 'Low', high: 'High', planning: 'Planning agent', direct: 'Worker agent', 'workspace-write': 'Accept edits' };
...
class WorkstreamWizard {
  constructor({ catalogs, models, lastUsed = null, repositories = null, rememberedRepositoryRoot = null } = {}) {
    ...
    this.models = Object.fromEntries(Object.entries(this.catalogs).map(([provider, values]) => [provider, values.map((model) => model.id)])); this.repositories = repositories || [{ root: null, label: '.', lastUsed }];
    ...
    this.values = { workerCapacity: 5 }; this.presets = {}; this.setPresets(lastUsed); this.steps = []; this.step = 0; this.selection = 0; this.selections = {}; this.scrolls = {}; this.input = ''; this.message = ''; this.history = []; this.lastHits = [];
    this.rebuildSteps();
    { const remembered = this.repositories.findIndex((item) => item.root === rememberedRepositoryRoot); this.selection = remembered >= 0 ? remembered : 0; }
  }
  ...
  rebuildSteps() { const type = this.values.sessionType; const setup = type === 'direct' ? DIRECT_STEPS : type === 'planning' ? PLANNING_STEPS : []; this.steps = ['repository', 'sessionType', ...(type ? ['preset'] : []), ...setup, ...(type ? ['confirmation'] : [])]; }
  ...
  options() { const key = this.key(); if (key === 'repository') return this.repositories.map((item) => item.label); if (key === 'sessionType') return ['planning', 'direct']; if (key === 'preset') return this.presets[this.values.sessionType] ? ['Last used', 'Customize'] : ['Customize']; if (key.endsWith('Provider')) return Object.keys(this.catalogs); if (key.endsWith('Model')) return [...this.modelOptions(this.values[`${this.prefix()}Provider`]), 'Type a model ID…']; if (key.endsWith('Effort')) return [...REASONING_EFFORTS]; if (key === 'confirmation') return ['Create session', 'Go back']; return []; }
  ...
  scroll(delta) { const maximum = Math.max(0, this.options().length - 5); this.scrolls[this.key()] = Math.max(0, Math.min(maximum, (this.scrolls[this.key()] || 0) + delta)); }
  ...
  visibleOptions() { const options = this.options(); const start = Math.max(0, Math.min(this.scrolls[this.key()] || 0, Math.max(0, options.length - 5))); return options.slice(start, start + 5).map((option, offset) => ({ option, index: start + offset })); }
  click(row, column) { const hit = this.lastHits.find((item) => item.row === row && column >= item.start && column <= item.end); if (!hit) return null; this.selection = hit.index; this.selections[this.key()] = hit.index; return this.choose(); }
  render() {
    ...
    if (this.message && !this.message.startsWith('Type the model')) lines.push(`${COLOR.error}${COLOR.bold}! ${this.message}${COLOR.reset}`); lines.push('', `${COLOR.accent}↑/↓ scroll  •  ← previous  •  Click choose  •  Enter continue  •  Esc back${COLOR.reset}`);
    const plain = lines.map((line) => line.replace(/\u001b\[[0-?]*[ -/]*[@-~]/gu, '')); this.lastHits = []; let from = 0; for (const { option, index } of this.visibleOptions()) { const shown = this.optionLabel(option); const row = plain.findIndex((line, lineIndex) => lineIndex >= from && line.includes(shown)); if (row < 0) continue; const start = plain[row].indexOf(shown) + 1; this.lastHits.push({ type: 'wizard-option', key, index, row: row + 1, start, end: start + [...shown].length - 1 }); from = row + 1; }
    return lines.join('\n');
  }
}
