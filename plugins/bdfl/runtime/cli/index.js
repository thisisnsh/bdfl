...
const HELP = `Usage: bdfl [--dangerous|status|help|--version]\n\nRun bdfl with no arguments in a Git workspace to attach the tmux-native supervisor. tmux 3.2 or newer must be installed.\nUse --dangerous to bypass provider approvals and sandboxing for every agent in this run.\n\nOne focused agent fills the terminal. Click a global agent tab or press C-b then Left/Right (or h/l) to cycle through every open agent. C-b o toggles a tiled overview of the current session; in overview, prefixed arrows move spatially and Enter returns to the selected agent. C-b n/p/s/r opens New, Plans, Sessions, or Reviews; uppercase aliases also work. C-b x pauses the active agent and selects a fallback. C-b q opens shutdown confirmation and C-b ? shows all controls. C-b [ enters copy mode. Unprefixed keys always reach the provider. Hold Shift while dragging to select native terminal text.\nIn Sessions, d deletes one managed agent and D deletes its whole session. In Plans, d deletes one plan and D deletes plans for the selected session. Deletions require Enter confirmation.`;
