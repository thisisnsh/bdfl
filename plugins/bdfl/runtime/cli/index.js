...
const HELP = `Usage: bdfl [--dangerous|status|help|--version]\n\nRun bdfl with no arguments in a Git workspace to open the foreground terminal supervisor.\nUse --dangerous to bypass provider approvals and sandboxing for every agent in this run.\n\nClick an open agent in the bottom rail to switch. Selecting a saved agent in Sessions resumes it in one action without losing its history.\nTop actions open New, Plans, Sessions, and Reviews; Close pauses only the selected agent. Clicks also work.\nOn a Sessions parent, r renames, d deletes that session, and D deletes all sessions. On Plans, r renames, d deletes the selected plan, and D deletes all plans. Deletions require Enter confirmation; Esc cancels. Ctrl+C twice quits.`;
