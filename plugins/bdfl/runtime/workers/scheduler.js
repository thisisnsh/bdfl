...
class WorkerScheduler {
  constructor(root, { store, lineage, launcher, worktrees, now = () => new Date(), id = () => crypto.randomUUID() } = {}) { this.root = path.resolve(root); this.store = store; this.lineage = lineage; this.launcher = launcher; this.worktrees = worktrees; this.now = now; this.id = id; }
  ...
  complete(id, chunkId, result) { const execution = this.load(id); const chunk = execution.chunks.find((item) => item.id === chunkId); if (!chunk || !ACTIVE.has(chunk.status)) throw new Error(`Chunk is not active: ${chunkId}`); const summary = `${result.summary || ''}`.slice(0, 800); const attempt = chunk.attempts.at(-1); Object.assign(attempt, { completedAt: this.now().toISOString(), result: result.state, summary }); Object.assign(chunk, { status: result.state === 'pass' ? 'review' : result.state === 'blocked' ? 'waiting' : 'failed', summary, commit: result.commit || attempt.commit, changedPaths: result.changedPaths || [], checks: result.checks || [] }); execution.events.push({ type: `worker.${chunk.status}`, chunkId, at: this.now().toISOString() }); this.save(execution); this.recalculate(id); return chunk; }
