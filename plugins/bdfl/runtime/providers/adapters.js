...
const ROLE =
  "You are this workstream's read-only delegator. Use bdfl-plan whenever creating or revising an implementation plan. Define the smallest useful dependency graph; do not create work merely to fill worker capacity. All implementation must run through approved BDFL workers.";
...
function codexRuntime(provider) {
...
function stripOwnedArgs(argv = [], provider) {
  ...
  if (provider === 'ollama') {
    for (const flag of ['-p', '--profile', '--local-provider']) pairs.add(flag);
    for (const flag of ['--oss', '--yes']) switches.add(flag);
  }
  const controlledConfig = new Set(['model', 'model_reasoning_effort']);
  const result = [];
  for (let index = 0; index < argv.length; index += 1) {
    const argument = argv[index];
    const pair = [...pairs].find((flag) => argument === flag || argument.startsWith(`${flag}=`));
    if (pair) {
      if (argument === pair) index += 1;
      continue;
    }
    if (switches.has(argument)) continue;
    if (codex && (argument === '-c' || argument === '--config') && index + 1 < argv.length) {
      const config = argv[index + 1];
      const key = config.split('=', 1)[0];
      if (
        controlledConfig.has(key) ||
        (provider === 'ollama' &&
          (key === 'profile' ||
            key === 'model_provider' ||
...
function codexSandboxOverride(argv = []) {
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (
      ...
      return true;
    if ((argument === '-c' || argument === '--config') && argv[index + 1]?.split('=', 1)[0] === 'sandbox_mode')
      return true;
    if (
      (argument.startsWith('-c=') || argument.startsWith('--config=')) &&
      argument.slice(argument.indexOf('=') + 1).split('=', 1)[0] === 'sandbox_mode'
    )
      return true;
  }
  return false;
...
function stripCodexSecurity(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument.startsWith('-c=') || argument.startsWith('--config=')) {
      ...
      const key = config.split('=', 1)[0];
      if (key === 'sandbox_mode' || key === 'approval_policy') continue;
    }
    result.push(argument);
  }
  return result;
...
function codexArgs(profile, options, { model = true, provider = 'codex' } = {}) {
  const custom = options.dangerous
    ? stripCodexSecurity(stripOwnedArgs(profile.argv, provider))
    : stripOwnedArgs(profile.argv, provider);
  const permission = options.dangerous
    ? ['--dangerously-bypass-approvals-and-sandbox']
    : !codexSandboxOverride(custom)
      ? ['--sandbox', codexSandbox(options.permissionMode || 'read-only')]
      : [];
  const common = [
    ...custom,
    '--no-alt-screen',
    ...
    ...permission,
    '-c',
    `tui.notifications=${JSON.stringify(ATTENTION_EVENTS)}`,
    '-c',
  ...
  if (options.bridge) {
    ...
    common.push(
      '-c',
      `mcp_servers.bdfl.command=${JSON.stringify(options.bridge.command)}`,
      '-c',
      `mcp_servers.bdfl.args=${JSON.stringify(options.bridge.args)}`,
      '-c',
      'mcp_servers.bdfl.required=true',
      '-c',
      `mcp_servers.bdfl.enabled_tools=${JSON.stringify(tools)}`,
      '-c',
      'mcp_servers.bdfl.default_tools_approval_mode="approve"'
    );
...
function buildCodex(profile, options) {
  return { command: 'codex', args: codexArgs(profile, options), env: TERMINAL_ENV };
}
function buildOllama(profile, options) {
  return {
    command: 'ollama',
    args: [
      'launch',
      ...
      '--yes',
      '--',
      ...codexArgs(profile, options, { model: false, provider: 'ollama' })
    ],
    env: TERMINAL_ENV
  };
}
...
function buildClaude(profile, options) {
  const custom = options.dangerous
    ? stripClaudePermissions(stripOwnedArgs(profile.argv, 'claude'))
    : stripOwnedArgs(profile.argv, 'claude');
  const defaultPermission = options.role === 'delegator' || options.role === 'verifier' ? 'manual' : 'acceptEdits';
  const permission = options.dangerous
    ? ['--dangerously-skip-permissions']
    : !claudePermissionOverride(custom)
      ? ['--permission-mode', defaultPermission]
      : [];
  const common = [
    ...custom,
  ...
  if (options.mcpConfig)
    common.push(
      ...
      '--strict-mcp-config',
      '--allowedTools',
      ...(options.allowedTools || ['mcp__bdfl__bdfl_workers'])
    );
  const attentionHook = {
    type: 'command',
    command: process.execPath,
    args: [path.resolve(options.attentionHelper || path.join(__dirname, 'attention-hook.js'))]
  };
  const settings = {
    hooks: {
      Stop: [{ hooks: [attentionHook] }],
      Notification: [{ matcher: CLAUDE_NOTIFICATION_EVENTS.join('|'), hooks: [attentionHook] }]
    }
  };
...
function buildLaunch(profileValue, options = {}) {
  ...
  const invocation =
    profile.provider === 'codex'
      ? buildCodex(profile, options)
      : profile.provider === 'ollama'
        ? buildOllama(profile, options)
        : buildClaude(profile, options);
  return {
    ...invocation,
    cwd: options.cwd,
    roleInstruction: options.role === 'delegator' ? ROLE : options.roleInstruction || null
  };
}
...
function pluginDestination(root, sessionId) {
  return path.join(root, '.bdfl', 'sessions', sessionId, 'plugin');
}

module.exports = {
  ROLE,
  ATTENTION_EVENTS,
  CLAUDE_NOTIFICATION_EVENTS,
  codexSandbox,
  codexRuntime,
  stripOwnedArgs,
  buildLaunch,
