...
class ControlServer {
  ...
  issue(scope) { for (const [existing, capability] of this.capabilities) if (capability.sessionId === scope.sessionId) this.capabilities.delete(existing); const token = crypto.randomBytes(32).toString('base64url'); this.capabilities.set(token, Object.freeze({ ...scope })); return { schema: 1, url: `http://${this.host}:${this.port}/rpc`, token, scope: { ...scope }, issuedAt: new Date().toISOString() }; }
  ...
  checkHeartbeats(now = Date.now()) { for (const [proxyId, proxy] of this.proxies) if (!proxy.lost && now - proxy.lastSeen > this.heartbeatTimeout) { proxy.lost = true; this.proxies.delete(proxyId); this.onProxyLost?.(proxy.capability.sessionId, { proxyId, lastSeen: proxy.lastSeen }); } }
