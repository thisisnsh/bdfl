...
class WorkstreamWizard {
  ...
  move(delta) { const length = this.options().length; if (length) { this.selection = (this.selection + delta + length) % length; this.selections[this.key()] = this.selection; const start = this.scrolls[this.key()] || 0; if (this.selection < start) this.scrolls[this.key()] = this.selection; else if (this.selection >= start + 5) this.scrolls[this.key()] = this.selection - 4; } }
  ...
  render() {
    ...
    if (key === 'repository' || key === 'sessionType') {
      const repositoryActive = key === 'repository'; const repository = this.answer('repository'); const type = this.answer('sessionType');
      lines.push(`${repositoryActive ? `${COLOR.white}${COLOR.bold}○ Repository${COLOR.reset}` : `${COLOR.done}✓ Repository${COLOR.reset}  ${COLOR.bold}${COLOR.white}${repository}${COLOR.reset}`}`);
      if (repositoryActive) activeDetails();
      lines.push('', `${!repositoryActive ? `${COLOR.white}${COLOR.bold}○ Session type${COLOR.reset}` : `${COLOR.dim}○ Session type  ${type}${COLOR.reset}`}`);
      if (!repositoryActive) activeDetails();
    }
    ...
    if (this.message && !this.message.startsWith('Type the model')) lines.push(`${COLOR.error}${COLOR.bold}! ${this.message}${COLOR.reset}`); lines.push('', `${COLOR.accent}↑/↓ choose  •  ← previous  •  Enter continue  •  Esc back${COLOR.reset}`);
    this.lastHits = [];
    return lines.join('\n');
  }
}
