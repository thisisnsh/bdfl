...
function inputTokens(value) { return `${value}`.match(/\u001b\[[0-9]+(?:;[0-9]+)*u|\u001b\[[A-D]|./gsu) || []; }
...
class TerminalSupervisor {
  ...
  sessionPickerLines() { const streams = this.sessionPickerItems(); this.syncSessionPicker(streams); return [`${COLORS.bold}${COLORS.yellow}Your sessions${COLORS.reset}`, `${COLORS.gray}Open a running session or resume one you closed earlier.${COLORS.reset}`, '', ...streams.flatMap((stream, index) => { const provider = display(stream.delegatorProfile?.provider || 'bdfl'); const originalIndex = this.workspace.workstreams.findIndex((item) => item.id === stream.id); const sequence = stream.providerSequence || this.workspace.workstreams.slice(0, originalIndex + 1).filter((item) => item.delegatorProfile?.provider === stream.delegatorProfile?.provider).length; const selected = index === this.sessionPicker.index; const deleting = this.sessionPicker.confirmDelete === stream.id; const title = `${provider} ${sequence}`; const status = deleting ? `${COLORS.red}${COLORS.bold}Delete permanently?${COLORS.reset}` : stream.status === 'closed' ? `${COLORS.yellow}Ready to resume${COLORS.reset}` : `${COLORS.green}Running${COLORS.reset}`; const description = deleting ? `${COLORS.red}Press Enter to confirm · Esc to keep this session${COLORS.reset}` : `${COLORS.gray}${stream.title || path.basename(this.root)} · ${selected ? 'Enter open · d delete' : stream.status === 'closed' ? 'Saved provider session' : 'Session is available'}${COLORS.reset}`; const agents = this.workspace.sessions.filter((session) => session.workstreamId === stream.id).sort((a, b) => (a.paneNumber || 0) - (b.paneNumber || 0)); const children = agents.length ? agents.map((agent, agentIndex) => { const connector = agentIndex === agents.length - 1 ? '└─' : '├─'; const role = agent.role === 'delegator' ? 'Planning agent' : agent.role === 'worker' ? 'Worker agent' : `${display(agent.role)} agent`; const agentProvider = display(agent.profile?.provider || stream.delegatorProfile?.provider || 'agent'); const agentStatus = agent.status === 'running' ? `${COLORS.green}Running${COLORS.reset}` : agent.explicitlyClosed ? `${COLORS.gray}Closed${COLORS.reset}` : `${COLORS.yellow}Saved${COLORS.reset}`; const shortcut = agents.length > 1 ? `[${agent.paneNumber || agentIndex + 1}] ` : ''; return `${COLORS.gray}   ${connector}${COLORS.reset} ${COLORS.bold}${COLORS.white}${shortcut}${role}${COLORS.reset} ${COLORS.gray}· ${agentProvider}${COLORS.reset}  ${agentStatus}`; }) : [`${COLORS.gray}   └─ No agents saved${COLORS.reset}`]; return [`${selected ? `${COLORS.inverse}${COLORS.bold} › ${title} ${COLORS.reset}` : `   ${COLORS.bold}${COLORS.white}${title}${COLORS.reset}`}  ${status}`, `   ${description}`, ...children, '']; }), `${COLORS.cyan}↑/↓ choose${COLORS.reset}${COLORS.gray}  •  Enter open  •  d delete  •  Esc/Ctrl+] back${COLORS.reset}`]; }
  ...
  start() {
    ...
    const handle = (value) => {
      ...
      if (isToggleKey(value)) {
        ...
        else { const selected = this.navigation.panes()[this.navigation.pane]; if (selected) { try { this.sessions.open(selected.id, this.dimensions()); this.error = null; this.navigation.rail = 'content'; } catch (error) { this.error = error.message; } } }
        return this.draw();
      }
  ...
  stop() { this.running = false; this.sessions.shutdown?.(); this.input.off('data', this.onData); this.input.setRawMode?.(false); this.input.pause(); this.output.write(`${ESC}?25h${ESC}?1049l`); this.release(); }
}

module.exports = { ACTIONS, availableActions, COLORS, width, fit, inputTokens, isToggleKey, Navigation, TerminalRenderer, TerminalSupervisor };
