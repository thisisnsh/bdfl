...
class TerminalSupervisor {
  ...
  stop() { this.running = false; this.sessions.shutdown?.(); this.input.off('data', this.onData); this.input.setRawMode?.(false); this.input.pause(); this.output.write(`${terminalTitle(path.basename(this.root), false)}${ESC}?25h${ESC}?1049l`); this.release(); }
}

module.exports = { ACTIONS, availableActions, COLORS, width, fit, inputTokens, isToggleKey, sanitizeTerminalTitle, hasOpenAttention, terminalTitle, Navigation, TerminalRenderer, TerminalSupervisor };
