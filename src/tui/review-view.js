...
function width(value) {
  return [...`${value}`.replace(/\u001b\[[0-9;?]*[A-Za-z]/gu, '')].reduce(
...
function patchLineType(line) {
  if (
    /^(?:diff --git |index |similarity index |dissimilarity index |rename (?:from|to) |copy (?:from|to) |new file mode |deleted file mode |old mode |new mode |--- |\+\+\+ )/u.test(
      line
    )
  )
    return 'file';
  ...
  return `${value ?? ''}`
    ...
    .map((text, index) => {
      ...
      if (paths) {
        ...
        oldPath = displayPath(text.replace(/^(?:rename|copy) from /u, '')) || oldPath;
      else if (text.startsWith('rename to ') || text.startsWith('copy to ')) {
        newPath = displayPath(text.replace(/^(?:rename|copy) to /u, '')) || newPath;
        file = newPath || oldPath;
      } else if (text.startsWith('--- ')) {
        oldPath = displayPath(text.slice(4));
        file = newPath || oldPath || file;
      } else if (text.startsWith('+++ ')) {
        newPath = displayPath(text.slice(4));
        file = newPath || oldPath || file;
      }
...
function selectionContains(selection, row) {
  return (
    selection.file === row.file &&
    selection.hunk === row.hunk &&
    row.sourceLine >= selection.startLine &&
    row.sourceLine <= selection.endLine
  );
}
...
function renderPatch(value, columns, selections = [], preview = []) {
  let visualLine = 0;
  return parsePatch(value).flatMap((source) =>
    wrapLine(source.text, Math.max(8, columns)).map((text, wrapIndex) => {
...
function stateDescriptor(item = {}) {
  ...
  if (item.phase?.label && ['verification-repair', 'integration-conflict', 'integration-checking'].includes(status))
    return { label: item.phase.label, tone: 'yellow', actions: [] };
  ...
  if (['waiting', 'blocked'].includes(status))
    return { label: 'Waiting for response', tone: 'yellow', actions: ['feedback'] };
  if (status === 'running' && item.attention) return { label: 'Needs response', tone: 'yellow', actions: ['feedback'] };
  if (status === 'running' && feedback.length)
    return { label: 'Feedback sent · Revising', tone: 'yellow', actions: [] };
  if (['checking', 'integration-checking'].includes(status)) return { label: 'Checking', tone: 'yellow', actions: [] };
  if (status === 'verification-repair') return { label: 'Verification repair', tone: 'yellow', actions: [] };
  ...
  if (['integration-queued', 'integrating'].includes(status))
    return { label: 'Integrating', tone: 'yellow', actions: [] };
  ...
  if (status === 'verification-failed')
    return { label: 'Verification failed', tone: 'red', actions: ['remedy', 'feedback', 'override'] };
...
function feedbackRows(history, columns) {
  ...
  return history.flatMap((entry) => {
    const rows = [
      '',
      `${COLORS.yellow}Feedback${entry.at ? ` · ${entry.at}` : ''}${COLORS.reset}`,
      ...wrapLine(entry.message || '', columns)
    ];
    for (const selection of entry.selections || []) {
      rows.push(
        `${COLORS.cyan}${selection.file || '(unknown file)'} · ${selection.hunk || '(unknown hunk)'} · lines ${selection.startLine}-${selection.endLine}${COLORS.reset}`
      );
...
class ReviewView {
  ...
  reconcile(item, resetDrag = false) {
    ...
    if (state.confirmation && !stateDescriptor(this.item).actions.includes(state.confirmation))
  ...
  open(item) {
    return this.reconcile(item, true);
  }
  update(item) {
    return this.reconcile(item);
  }
  selections() {
    return this.state().selections.map((selection) => ({ ...selection }));
  }
  ...
  patchRows() {
    return renderPatch(this.item?.diff || '', this.columns, this.state().selections, this.previewSelections());
  }

  bodyRows() {
    const summary = wrapLine(this.item?.summary || 'No summary supplied.', this.columns).map((text) => ({
      text,
      styled: text
    }));
    const diff = this.patchRows().map((row) => ({ ...row, patch: true }));
    const checks = (this.item?.checkResults || this.item?.checks || [])
      .flatMap((check) => [
        `${check.ok ? '✓' : '✗'} ${(check.command || []).join(' ')}`,
        ...`${check.output || ''}`.split('\n').filter(Boolean)
      ])
      .flatMap((text) => wrapLine(text, this.columns))
      .map((text) => ({ text, styled: text }));
    const feedback = feedbackRows(this.item?.feedback, this.columns).map((text) => ({ text, styled: text }));
    return [
      ...summary,
      { text: '', styled: '' },
      ...diff,
      ...(checks.length ? [{ text: '', styled: '' }, ...checks] : []),
      ...feedback
    ];
  }

  maximumScroll() {
    return Math.max(0, this.bodyRows().length - this.viewportHeight);
  }
  clampScroll() {
    const state = this.state();
    state.scroll = Math.min(
      Math.max(0, Number.isFinite(state.scroll) ? Math.trunc(state.scroll) : 0),
      this.maximumScroll()
    );
    return state.scroll;
  }
  scroll(delta) {
    this.state().scroll += Number.isFinite(delta) ? Math.trunc(delta) : 0;
    return this.clampScroll();
  }
  scrollTo(offset) {
    this.state().scroll = Number.isFinite(offset) ? Math.trunc(offset) : 0;
    return this.clampScroll();
  }
  page(direction) {
    return this.scroll((direction < 0 ? -1 : 1) * this.viewportHeight);
  }
  resize(columns, viewportHeight = this.viewportHeight) {
    this.columns = Math.max(8, columns);
    this.viewportHeight = Math.max(1, viewportHeight);
    this.clampScroll();
    return this;
  ...
  render({ bodyTop = 1, bodyLeft = 1, bodyWidth = this.columns } = {}) {
    ...
    const rows = body
      .slice(offset, offset + this.viewportHeight)
      .map((row, index) => ({ ...row, screenRow: bodyTop + index, bodyIndex: offset + index }));
    const hits = rows
      .filter((row) => row.patch)
      .map((row) => ({
        row: row.screenRow,
        start: bodyLeft,
        end: bodyLeft + Math.max(0, bodyWidth - 1),
        visualLine: row.visualLine
      }));
    const frame = {
      lines: rows.map((row) => row.styled),
      rows,
      hits,
      offset,
      maxScroll: this.maximumScroll(),
      status: stateDescriptor(this.item),
  ...
  hit(row, column) {
    return this.lastFrame?.hits.find((hit) => hit.row === row && column >= hit.start && column <= hit.end) || null;
  }
  ...
  rangesForVisualSpan(first, last) {
    ...
    for (const source of sources.filter((row) => row.file && row.hunk && row.sourceLine > 0 && row.sourceText.trim())) {
      ...
      if (
        ...
        previous.hunk === source.hunk &&
        previous.endLine + 1 === source.sourceLine
      ) {
        ...
        ranges.push({
          file: source.file,
          hunk: source.hunk,
          startLine: source.sourceLine,
          endLine: source.sourceLine,
  ...
  beginSelection(visualLine) {
    if (!Number.isInteger(visualLine)) return false;
    this.drag = { anchor: visualLine, current: visualLine };
    return true;
  }
  extendSelection(visualLine) {
    if (!this.drag || !Number.isInteger(visualLine)) return false;
    this.drag.current = visualLine;
    return true;
  }
  ...
  handleMouse(mouse) {
    ...
    if (mouse.final === 'M' && (mouse.button & 32) !== 0 && this.drag) {
      if (hit) this.extendSelection(hit.visualLine);
      return true;
    }
    if (mouse.final === 'm' && this.drag) {
      this.finishSelection(hit?.visualLine);
      return true;
    }
    return false;
  }
  ...
  removeSelection(index) {
    if (index < 0 || index >= this.state().selections.length) return false;
    this.state().selections.splice(index, 1);
    this.clampScroll();
    return true;
  }
  removeLastSelection() {
    return this.removeSelection(this.state().selections.length - 1);
  }
  clearSelections() {
  ...
  feedback(message) {
    return { message: `${message ?? ''}`, selections: this.selections() };
  }

  requestAction(action, guidance = '') {
    const allowed = stateDescriptor(this.item).actions;
    if (!allowed.includes(action)) return { action: null };
    if (action === 'remedy' || action === 'retry' || action === 'override' || action === 'integrate') {
      this.state().confirmation = action;
      this.state().confirmationChoice = action === 'override' ? 'cancel' : 'confirm';
      this.state().confirmationGuidance = `${guidance || ''}`.trim();
  ...
  cancelConfirmation() {
    ...
    this.state().confirmationChoice = null;
    this.state().confirmationGuidance = '';
  }
  moveConfirmation() {
    if (!this.state().confirmation) return null;
    this.state().confirmationChoice = this.state().confirmationChoice === 'confirm' ? 'cancel' : 'confirm';
    return this.state().confirmationChoice;
  }
  confirmAction() {
    const action = this.state().confirmation;
    const guidance = this.state().confirmationGuidance;
    const confirmed = this.state().confirmationChoice === 'confirm';
    this.cancelConfirmation();
    if (!action || !confirmed) return { action: null };
    return guidance ? { action, guidance } : { action };
  }
}
