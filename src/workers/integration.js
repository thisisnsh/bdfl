...
class IntegrationCoordinator {
  ...
  requestValidationRepair(execution, failed, checkResults) {
    ...
    execution.status = 'integration-conflict'; execution.integration = { ...integration, conflict: result, worker, allowedPaths, validationRepairAttempts: attempts, checkResults }; execution.verification = { state: 'fail', summary: `Reconciled target validation failed; visible repair agent ${attempts}/${MAX_RECONCILIATION_REPAIR_ATTEMPTS} is fixing ${command}`.slice(0, 800), affectedChunkIds: [], completedAt: this.now().toISOString() }; this.scheduler.save(execution); return { state: 'conflict', queued: true, worker, validationRepair: true };
  }
  ...
  resumeIntegrationQueue() {
    ...
    for (const current of executions) {
      ...
      else if (execution.status === 'verification-failed' && execution.integration?.queue && execution.integration?.lastReconciliation?.commit && /^Reconciled target validation failed/.test(execution.verification?.summary || '') && (execution.integration.validationRepairAttempts || 0) < MAX_RECONCILIATION_REPAIR_ATTEMPTS) { execution.status = 'integration-queued'; execution.integration.queue = { ...execution.integration.queue, active: false, resumedAt: this.now().toISOString(), retryReason: 'Retrying a reconciled validation failure with a visible repair agent' }; this.scheduler.save(execution); }
    }
...
module.exports = { IntegrationCoordinator, MAX_VERIFIER_ATTEMPTS, MAX_RECONCILIATION_REPAIR_ATTEMPTS };
