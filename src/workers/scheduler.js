...
class WorkerScheduler {
  ...
  recalculate(id) {
    ...
    for (const chunk of execution.chunks) {
      ...
      const predecessors = this.ancestors(execution, chunk); const commits = predecessors.map((item) => item.commit).filter(Boolean); const base = commits.length && this.worktrees?.composeBase ? this.worktrees.composeBase(execution.id, chunk.id, chunk.attempts.length + 1, execution.baseline, commits) : commits.at(-1) || execution.baseline;
  ...
  materializeContext(execution, chunk) { const directory = path.join(this.root, '.bdfl', 'workers', execution.id, chunk.id, 'context'); fs.mkdirSync(path.join(directory, 'dependency-results'), { recursive: true }); atomicWrite(path.join(directory, 'shared.md'), this.lineage.readSection(execution.planId, execution.version, 'shared')); atomicWrite(path.join(directory, 'chunk.md'), this.lineage.readSection(execution.planId, execution.version, chunk.id)); for (const predecessor of this.ancestors(execution, chunk)) atomicWrite(path.join(directory, 'dependency-results', `${predecessor.id}.json`), `${JSON.stringify({ id: predecessor.id, commit: predecessor.commit, summary: predecessor.summary }, null, 2)}\n`); atomicWrite(path.join(directory, 'execution.json'), `${JSON.stringify({ executionId: execution.id, planId: execution.planId, version: execution.version, chunkId: chunk.id, paths: chunk.paths, checks: chunk.checks, base: chunk.attempts.at(-1).base }, null, 2)}\n`); return directory; }
