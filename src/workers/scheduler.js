...
class WorkerScheduler {
  ...
  recalculate(id) {
    ...
    for (const chunk of execution.chunks) {
      ...
      const source = this.lineage.readSection(execution.planId, execution.version, chunk.id); chunk.title ||= source.match(/^##\s+(.+?)\s*$/mu)?.[1]?.trim() || null; const taskSnippet = workerTaskSnippet(source, chunk.id); chunk.taskSnippet = taskSnippet; const context = this.materializeContext(execution, chunk); const launched = this.launcher?.({ execution, chunk, attempt, context, profile: execution.profile, taskSnippet });
  ...
  status(id) { const execution = this.load(id); return { id, planId: execution.planId, version: execution.version, status: execution.status, capacity: execution.capacity, chunks: execution.chunks.map(({ id: chunkId, title, status, commit, dependsOn, taskSnippet, attempts }) => ({ id: chunkId, title: title || taskSnippet || chunkId, status, commit, dependsOn, taskSnippet: taskSnippet || null, sessionId: attempts.at(-1)?.sessionId || null })), paths: { execution: this.executionFile(id, execution.repositoryRoot) } }; }
