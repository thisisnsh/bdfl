...
function unquote(value) {
  return `${value || ''}`.replace(/^(?:"(.*)"|'(.*)')$/u, (_match, double, single) => double ?? single);
}
...
function hasDangerousProviderArgs(provider, argv = []) {
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (
      ...
      return true;
    if (provider === 'claude' && unquote(optionValue(argv, index, ['--permission-mode'])) === 'bypassPermissions')
      return true;
    if (provider !== 'claude' && unquote(optionValue(argv, index, ['-s', '--sandbox'])) === 'danger-full-access')
      return true;
    const config = optionValue(argv, index, ['-c', '--config']);
    const separator = config?.indexOf('=') ?? -1;
    if (
      provider !== 'claude' &&
      separator > 0 &&
      config.slice(0, separator) === 'sandbox_mode' &&
      unquote(config.slice(separator + 1)) === 'danger-full-access'
    )
      return true;
  }
  return false;
...
function tokenizeCommand(command) {
  ...
  if (/[|;&<>`\n\r]/.test(command) || /\$\(|\$\{/.test(command))
    throw new Error('Shell syntax is not allowed in a provider profile');
  const argv = [];
  let token = '';
  let quote = null;
  ...
  if (argv.slice(1).some((argument) => argument.includes('=') && !argument.startsWith('-')))
    throw new Error('Environment prefixes are not allowed');
  ...
  if (argv.slice(1).some((argument) => forbidden.has(argument) || argument.startsWith('--settings=')))
    throw new Error('BDFL owns headless, resume, MCP, settings, and role flags');
  if (hasDangerousProviderArgs(argv[0], argv.slice(1)))
    throw new Error('Dangerous provider permissions require bdfl --dangerous');
...
function validateProfile(profile, { worker = false } = {}) {
  ...
  if (profile.argv) {
    ...
    if (hasDangerousProviderArgs(profile.provider, profile.argv))
      throw new Error('Dangerous provider permissions require bdfl --dangerous');
...
function validateWorkstreamConfig(value) {
  ...
  if (sessionType === 'direct')
    return { version: 1, sessionType, directProfile: validateProfile(value.directProfile, { worker: true }) };
  if (sessionType !== 'planning') throw new Error('Session type must be planning or direct');
  if (!Number.isInteger(value.workerCapacity) || value.workerCapacity < 1 || value.workerCapacity > 5)
    throw new Error('Worker capacity must be an integer from 1 to 5');
  return {
    version: 1,
    sessionType,
    delegatorProfile: validateProfile(value.delegatorProfile),
    workerProfile: validateProfile(value.workerProfile, { worker: true }),
