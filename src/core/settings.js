...
function validateCatalog(catalog) {
  ...
  return catalog.map((entry) => {
    if (!['claude', 'codex'].includes(entry.provider) || !entry.model) throw new Error('Invalid discovered model catalog entry');
    const { efforts: _efforts, defaultEffort: _defaultEffort, ...model } = entry;
    return model;
  });
}

function withoutLegacyEffort(value) {
  if (typeof value !== 'string') return value;
  const last = value.lastIndexOf(':');
  return /^(?:low|medium|high|xhigh|max|ultra)$/i.test(value.slice(last + 1)) ? value.slice(0, last) : value;
}

function validateSettings(value) {
  if (!value || value.version !== 3) throw new Error('Settings version must be 3');
  ...
  for (const model of models) {
    ...
    if (!['claude', 'codex'].includes(parsed.provider)) throw new Error(`Unsupported configured provider: ${parsed.provider}`);
  }
  ...
  return Object.freeze({ ...value, defaultModel: value.defaultModel || null, models: Object.freeze(models), discoveredModels: Object.freeze([...value.discoveredModels]), customModels: Object.freeze([...value.customModels]), modelCatalog: Object.freeze(catalog.map((entry) => Object.freeze({ ...entry }))) });
}

function migrateSettings(user = {}) {
  if (user.version === 3) return { ...user, customModels: [...(user.customModels || [])] };
  const legacyModels = Array.isArray(user.models) ? user.models : [];
  return {
    version: 3,
    defaultModel: withoutLegacyEffort(user.defaultModel) || null,
    customModels: [...new Set([...(user.customModels || []), ...legacyModels].map(withoutLegacyEffort))],
...
function resolveSettings(user, catalog, invokingHost) {
  ...
  const installedProviders = new Set(normalizedCatalog.map((entry) => entry.provider));
  const customModels = [...new Set((migrated.customModels || []).filter((specification) => {
    try { const parsed = parseModelSpec(specification); return installedProviders.has(parsed.provider) && !discoveredKeys.has(`${parsed.provider}:${parsed.model}`); }
    catch { return false; }
  }))];
  ...
  if (!defaultModel && preferred) defaultModel = `${preferred.provider}:${preferred.model}`;
  if (!defaultModel) defaultModel = customModels[0] || null;
  return validateSettings({
    version: 3, defaultModel, models, discoveredModels, customModels, modelCatalog: normalizedCatalog,
