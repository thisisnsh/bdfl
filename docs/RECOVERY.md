...
- `C-b q` opens an explicit confirmation before normal shutdown: BDFL snapshots panes, stops providers, kills only its private tmux server, closes the daemon, and releases runtime files. Closing the terminal client or losing the daemon leaves tmux panes running; the next `bdfl` reconnects and reconciles them. (`C-b Q` remains an alias.)
- Native Sessions, Plans, and Reviews overlays reconstruct themselves from versioned daemon snapshots and durable files, not model context. Live refreshes preserve the selected durable ID.
