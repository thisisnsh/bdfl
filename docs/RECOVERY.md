...
- `C-b Q` performs a normal shutdown: it snapshots panes, stops providers, kills only BDFL's private tmux server, closes the daemon, and releases its runtime files. Closing the terminal client or losing the daemon leaves tmux panes running; the next `bdfl` reconnects and reconciles them.
