...
Accepted commits apply to an integration worktree in dependency order. Conflicts create worker work; the delegator never edits code. A fresh verifier reviews the consolidated diff and global checks without implementing changes, and verifier sessions remain visible in the agent rail and Sessions page. Final integration requires the original target branch and a clean working tree. Requests enter a durable per-repository integration queue, and only its oldest active item may inspect or advance that target. When the target HEAD has advanced through committed descendant history, BDFL cherry-picks the approved result in a disposable reconciliation worktree. A conflict launches a visible workspace-write reconciliation agent that preserves both committed intents and reports immediately without rerunning validation. BDFL persists an `integration-checking` phase, runs each global check in a background process with a hard deadline, and keeps the supervisor and MCP bridge responsive. It retains the failing output tail; a failed combined tree remains queued and launches a visible validation-repair agent for up to three attempts instead of being discarded. Interrupted checking and legacy reconciliation failures resume from durable state. Passing checks launch a fresh read-only verifier before BDFL fast-forwards the target with one workstream commit and releases the next queue item. Rewritten history, exhausted repair, and verification failures leave the target untouched.
