...
BDFL launches interactive agent CLIs and preserves their native authentication, settings inheritance, and session stores. Ollama uses its supported Codex integration: BDFL runs `ollama launch codex --model <id> --yes -- ...`, and the arguments after `--` carry the same managed Codex contract as a direct Codex session. Claude delegators and verifiers default to `manual`; Codex and Ollama-backed Codex delegators and verifiers default to the read-only sandbox. Delegators also receive the short BDFL role instruction and the session-only `bdfl-plan` skill. Workers receive only the approved clean shared contract, their chunk, dependency results, and execution metadata.
...
Worker access defaults to accept edits (`workspace-write` in the durable profile), so onboarding does not ask users to choose a permission mode. A worker CLI option can override that provider default and is also reused by the fresh verifier launched from the frozen worker profile.
