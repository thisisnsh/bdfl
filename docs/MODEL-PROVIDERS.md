...
BDFL launches interactive agent CLIs and preserves their native authentication, settings inheritance, and session stores. Ollama uses its supported Codex integration: BDFL runs `ollama launch codex --model <id> --yes -- ...`, and the arguments after `--` carry the same managed Codex contract as a direct Codex session. Claude delegators default to `manual`, while Codex and Ollama-backed Codex delegators default to the read-only sandbox. Isolated implementation workers and the durable execution agent use the worker profile's edit policy; verification remains a prompt-enforced read-only phase of that execution conversation. Delegators also receive the short BDFL role instruction and the session-only `bdfl-plan` skill. Every worker receives the complete frozen approved plan, explicit assignment manifest, dependency results, and validation contract, and must audit its chunk against that whole context.

Provider-native lifecycle notifications feed BDFL's persistent attention markers. Claude receives session-level `Stop` and input-waiting `Notification` hooks through an additional `--settings` value; a packaged Node helper returns an internal BEL through Claude's `terminalSequence` field. Direct Codex and Ollama-backed Codex sessions receive TUI notifications for completed turns, approval requests, and plan-mode prompts with the BEL method enabled regardless of terminal focus. BDFL consumes those BEL events internally: it does not add audible or desktop notifications. Repeated BEL events are coalesced after the first marker, and submitting new input re-arms attention for the next interaction.
...
Profiles accept optional CLI arguments. BDFL tokenizes them without a shell and stores them as an argv array. Ollama profile arguments are passed to Codex after the launcher's `--` separator. Safe native Claude permission options and Codex/Ollama sandbox and approval options override BDFL's role defaults. Dangerous flags, Claude `bypassPermissions`, and the Codex `danger-full-access` sandbox are rejected from profiles; launch the supervisor with `bdfl --dangerous` to opt every provider session into its native bypass mode for that run. BDFL also rejects pipes, redirection, substitutions, environment prefixes, headless modes, and flags it owns for model, provider/profile, effort, resume, MCP, hooks, and role injection. The complete accepted setup is saved locally and offered as **Last used** when the next workstream is created.

Worker access defaults to accept edits (`workspace-write` in the durable profile), so onboarding does not ask users to choose a permission mode. A worker CLI option can override that provider default and is also reused by the durable execution agent launched from the frozen worker profile.
